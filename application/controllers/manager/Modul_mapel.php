...
class Modul_mapel extends Member_Controller {
    ...
    public function index($page=null, $id=null){
        ...
        $query_image = $this->image_model->get_all();
        if($query_image->num_rows()>0){
        	$select_image = '';
        	$query_image = $query_image->result();
        	foreach ($query_image as $temp) {
        		$select_image = $select_image.'<option value="'.$temp->file_name.'">'.$temp->name.'</option>';
        	}

        }else{
        	$select_image = '<option value="10000000">KOSONG</option>';
        }

        $data['modul_id'] = $modul_id;
        $data['select_image'] = $select_image;
        $data['select_modul'] = $select;
        
        $this->template->display_admin($this->kelompok.'/mapel_view', 'Daftar Topik', $data);
    }
    ...
    function tambah(){
        ...
        $this->form_validation->set_rules('tambah-gambar', 'Gambar','required|strip_tags');
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $data['topik_image'] = $this->input->post('tambah-gambar', true);
            $data['topik_aktif'] = 1;
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
            ...
            $record[] = $jml_soal;
            $record[] = '<img src="../../uploads/images/'.$temp->topik_image.'">';
            // if($temp->topik_modul_id==10) {
            // 	$record[] = '<img src="../../uploads/images/'.$temp->topik_image.'">';
            // } else {
            // 	$record[] = 'No image';
            // }
